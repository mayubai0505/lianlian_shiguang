            ...
            function selectWriterRoute(mode, userText, history) {
                ...
                if (highFreedom.required) {
                    return {
                        ...
                        // API 暫時異常才 fallback；不是先讓 Gemini 試寫一次。
                        fallbackModelId:
                            mode === "resonance"
...
function parseRoleCommands(userInput, activeCharacters, currentFocusCharacter, charactersList) {
    ...
    return {
        ...
        `;
            ...
            - 角色台詞使用「」並自然分段。
            - 不得替玩家補出「」中的新台詞。
            - 正文通常約 900～1400 個中文字；若場景自然完成可略短，不要硬灌字。
       ...
       3. 首行含時間地點。`;
                   ...
                   res.on("close", () => {
                       if (!res.writableEnded) {
                          ...
                          const HISTORY_LIMIT =
                              chatMode === "resonance" ? 12 : // Premium：最近 6 輪 + RAG，避免 context 過肥
                                  ...
                                  console.log(
                                      ...
                                      JSON.stringify(currentMessages).length
                                                                           ...
                                                                           function compressResidualWriterArtifacts(rawText) {
                                                                               ...
                                                                               text = text.replace(/([\u3400-\u9fff])\1/g, (pair, char) => {
                                                                                   return allowedRepeats.has(pair) ? pair : char;
                                                                               });
...
const MAX_AI_RETRIES = 2;
                                                                               ...
                                                                               currentVoice = sanitizeWriterArtifacts(fixMojibake(currentVoice));
                                                                               ...
                                                                               const canRetryWriterQuality =
                                                                                   hasResidualWriterArtifacts &&
                                                                                   loopCount < MAX_LOOPS;

                                                                               if (
                                                                                   !canRetryWriterQuality &&
                                                                                   (
                                                                                       (
                                                                                           finalResponseText.length >= TARGET_LENGTH &&
                                                                                           hasCompleteStatus
                                                                               ...
                                                                               if (canRetryWriterQuality) {
                                                                                   console.warn(
                                                                                       `🔁 Final Quality Guard：第 ${loopCount} 份草稿仍有生成抖字，啟動一次完整重生。`,
                                                                                   );
                                                                               }
                                                                               ...
                                                                               const qualityRetryNote =
                                                                                   hasResidualWriterArtifacts
                                                                                       ? `
                                                                               【文字品質修復｜最高優先】
                                                                               上一份草稿出現模型生成抖字或殘片（例如不自然的單字重複、孤立字元或英文殘詞），該草稿已作廢。
                                                                               請重新生成完整回覆，不要修補或引用上一份草稿。
                                                                               輸出前自行檢查：正常中文疊字可保留，但不得出現「為為／力力／意意／顆顆」這類不自然重複。
                                                                               `
                                                                                       : "";
                                                                               ...
                                                                               1. 正文至少 ${TARGET_LENGTH} 個中文字，${config.writerTier === "grok_high_freedom" ? "建議控制在 650～900 字" : (chatMode === "resonance" ? "建議控制在 900～1400 字" : "建議控制在 700～1000 字")}；完成前不得提前結束。
